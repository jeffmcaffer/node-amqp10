...
var Builder = require('buffer-builder'),
  ...
  debugMessage = require('debug')('amqp10:link:message:sender'),
...
SenderLink.prototype.canSend = function () {
  if (this.state() !== 'attached') {
    debug('canSend(' + this.state() + ') = false');
    return false;
  }
...
SenderLink.prototype.send = function (msg, options) {
  debug('send(' + this.state() + ')');
  debugMessage('send(' + this.state() + '): ', msg);
  options = options || {};
  ...
  if (cbPolicy === putils.SenderCallbackPolicies.None) {
    ...
    if (!this.canSend()) {
      debug('deferring send (can\'t send)');
      debugMessage('deferring send (can\'t send): ', msg);
  ...
  return new Promise(function (resolve, reject) {
    var sendMessage = function (err) {
      ...
      debugMessage('sending: ', msg);
      var messageId;
      // _sendMessage may throw. If it does throw, assume there was something wrong in the
      // infrastructure (e.g., detached) and try again by adding it to the list of pending sends
      try {
        messageId = self._sendMessage(message, {
          deliveryTag: new Buffer(deliveryTag.toString())
        });
      } catch (error) {
    ...
    if (!sendMessage(null)) {
      debug('deferring send (sendMessage failed)');
      debugMessage('deferring send (sendMessage failed): ', msg);
      self._pendingSends.push(sendMessage);
    }
  });
};
...
SenderLink.prototype._dispatchPendingSends = function (err) {
  debug('dispatching pending sends', err ? err.message : '');
